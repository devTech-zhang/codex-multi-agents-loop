...
1. 进入 Codex 桌面端的插件页面，点击右上角 **Add marketplace**。
2. 来源填写 Git URL：`https://github.com/devTech-zhang/multi-agent-delivery-workflow.git`。
3. 分支填写 `main`。
4. 稀疏路径留空。
5. 在插件列表中安装并启用 `delivery-workflow`。
...
进入你的业务项目目录后，可以直接让 AI 调用插件工具完成初始化和创建项目，也可以使用 CLI：
...
> [!IMPORTANT]
> PRD 审批 Gate 默认只能由飞书卡片回调提交。Agent 不应在终端里代替用户选择“通过 / 拒绝”，也不应通过 MCP 替用户提交审批结果。
...
项目优先读取当前目录的 `workflow.config.json`。插件根目录的 `delivery-workflow.config.json` 只是默认模板，`config init` 会把它复制到业务项目目录。
...
敏感配置只放 `.env` 或环境变量，不要写入 `workflow.config.json`：
...
PRD v2 会被整理成带标题和表格结构的 XML，再通过 `lark-cli docs +create` 创建飞书文档，避免文档显示 `Untitled` 或 Markdown 表格渲染成纯文本。

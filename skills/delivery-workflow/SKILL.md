...
description: 使用通用软件交付 Workflow Worker 编排需求、PRD、评审、设计、开发、测试、上线和最终报告。适用于用户要新建交付项目、查询此项目状态、删除此项目、启动/推进/调试 delivery workflow 时。
---
...
- 用户说“查询此项目状态 / 查看当前项目状态 / 这个项目到哪一步了”：调用 `delivery_get_current_project_status`。
- 用户说“删除此项目 / 清理当前项目”：调用 `delivery_delete_current_project`。删除前默认备份 zip 到当前目录，除非用户明确要求跳过备份。
...
删除前默认在当前目录生成 zip 备份；用户明确不需要备份时才使用 `--no-backup`。

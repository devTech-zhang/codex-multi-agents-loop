...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="codex-multi-agents-loop")
    ...
    create_p.add_argument("--mode", default="auto", choices=["auto", "full_workflow", "single_agent_task", "multi_agent_task"])
    create_p.add_argument("--agent", action="append", dest="requested_agents")
    ...
    task_p = project_sub.add_parser("task")
    task_p.add_argument("--requirement", required=True)
    task_p.add_argument("--agent", required=True)
    ...
    memory_pack_p = memory_sub.add_parser("pack")
    memory_pack_p.add_argument("--agent")
    memory_pack_p.add_argument("--query")
    memory_pack_p.add_argument("--limit", type=int, default=6)
    memory_pack_p.add_argument("--max-chars", type=int, default=4000)
    memory_pull_p = memory_sub.add_parser("pull")
    memory_pull_p.add_argument("--agent")
    memory_pull_p.add_argument("--limit", type=int, default=6)
    memory_sync_p = memory_sub.add_parser("sync")
    memory_sync_p.add_argument("--run-id")
    memory_sync_p.add_argument("--agent")
    memory_sync_p.add_argument("--dry-run", action="store_true")
...
def _dispatch(args: argparse.Namespace, parser: argparse.ArgumentParser) -> int:
            ...
            print_json(
                {
                    ...
                    "project": create_project(
                        ...
                        mode=args.mode,
                        requested_agents=args.requested_agents,
                    ),
                }
            )
            return 0
        if args.project_command == "task":
            print_json({"ok": True, "project": create_agent_task(requirement=args.requirement, agent=args.agent, title=args.title)})
            return 0
    ...
    if args.command == "memory":
        if args.memory_command == "pack":
            print_json({"ok": True, "memory": memory_pack(agent=args.agent, query=args.query, limit=args.limit, max_chars=args.max_chars)})
            return 0
        if args.memory_command == "pull":
            print_json({"ok": True, "memory": pull_global_memory(agent=args.agent, limit=args.limit)})
            return 0
        if args.memory_command == "sync":
            print_json({"ok": True, "memory": sync_global_memory(run_id=args.run_id, agent=args.agent, dry_run=args.dry_run)})
            return 0
    parser.print_help()
    return 2

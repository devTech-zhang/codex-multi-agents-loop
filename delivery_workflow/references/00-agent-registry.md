...
- 不得自行替用户提交开发前资料确认 Gate；确认结果只来自用户在 Codex / Claude Code 中明确给出的继续或修改指令。
- `code_platforms.enable_agent_cli=false` 时，只生成任务包和结构化产物，不接管业务项目代码实现。
- 每个步骤只读取 workflow 声明的输入 artifact，只产出当前步骤声明的输出 artifact。
- 开发前资料被要求修改后，`product_manager` 在 `review-summary` 中读取 `development-doc-confirmation_gate.comment`，据此修订最终 PRD 和后续文档，不回退到 `prd-v1` 重新确认需求。
- 前后端、开发自测、QA 和 Bug 修复阶段必须执行真实命令；不能把未执行的任务包描述为已完成。
- QA 系统测试结果必须作为开发修复输入，开发修复报告必须作为 QA 回归输入；`qa-system-testing/qa-regression-testing → bug-fix → qa-regression-testing` 会循环到 bug 等级和数量满足质量门禁。
...
- `review-summary` 汇总多角色评审意见，生成最终 PRD。
- 如果存在被拒绝的 `development-doc-confirmation_gate`，优先处理拒绝理由，说明如何修订最终 PRD。
...
- 最终报告汇总范围、产物、开发、联调、测试、上线结论、风险和后续建议。
- 不绕过 workflow 自行推进 Gate。

### qa_engineer

负责 `smoke-test-case-design`、`qa-test-report` 和系统/回归测试判级要求。

- 开发前基于最终 PRD、UI 规范和技术方案设计冒烟测试用例。
- QA 测试报告基于真实系统测试和回归测试结果生成，不得只写摘要。
- 覆盖主流程、边界、异常、权限、兼容性、联调和回归范围。
- 输出可执行的冒烟清单、测试报告和验收标准。
...
- 必须读取最终 PRD、UI 规范、前后端方案、开发冒烟自测报告、QA 系统/回归测试结果、上一轮修复报告和开发结果。

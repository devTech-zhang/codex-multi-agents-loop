...
- 读取 `prd_v2`、`ui_design_spec`、前后端技术方案、开发自测报告、测试用例、开发结果和上一轮 `bug_fix_result`。
- 第一轮执行完整系统测试；如果存在上一轮 `bug_fix_result`，本轮必须作为回归测试，逐条验证开发修复报告中的 `fixed_bugs`，并说明是否通过回归。
...
- 测试报告是开发修复的输入，必须写清每个 bug 的唯一编号、责任侧建议、复现步骤和验证口径。
